...
function filterUnfundedOnly() {
    ...
    console.log("Number of Unfunded games:", unfundedGames.length); // Loging the unfunded games to the console.
}
...
function filterFundedOnly() {
    ...
    console.log("Number of Funded games:", fundedGames.length); // Loging the funded games to the console.
}
...
const informationString = `
  A total of $${totalFundsraised.toLocaleString()} has been raised
  for ${totalGames - numberOfUnfundedGames} games. Currently
  ${numberOfUnfundedGames > 0 ? `${numberOfUnfundedGames} game${numberOfUnfundedGames > 1 ? 's' : ''} remain${numberOfUnfundedGames > 1 ? '' : 's'} unfunded. We need your help to fund these amazing games!` : "All games are funded!"}
`;
...
// Adding the paragraph element to the descriptionContainer.
...
// Extracting the first word of each game's name
...
firstGameElement.innerHTML = `<p>${mostFundedGame.name}</p>`;
firstGameContainer.appendChild(firstGameElement);

// do the same for the runner up item
const secondGameElement = document.createElement("div");
secondGameElement.innerHTML = `<p>${secondMostFundedGame.name}</p>`;
